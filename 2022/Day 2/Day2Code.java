...
class Day1Code {
  static String[] reader() throws FileNotFoundException {
    return new Scanner(new File("Day2input.txt")).useDelimiter("\\Z").next().split("\n");
  }
  ...
  static void part1() throws FileNotFoundException {
    ...
    for(String s : f) {
      ...
      if((D.get(s.charAt(0))+1)%3 == D.get(s.charAt(2))-1) ans += 6;
      else if (D.get(s.charAt(0)) == D.get(s.charAt(2))-1) ans += 3;
    }
