...
def part2():
  ...
  for l in f:
    ...
    P.add(p)
    for i in range(3):
      R[i] = (min(p[i], R[i][0]), max(p[i], R[i][1]))
    ...
    for y in range(R[1][0], R[1][1] + 1):
      for z in range(R[2][0], R[2][1] + 1):
        ...
        while q:
          ...
          if p in v or p in P:
            continue
          if p[0] not in range(R[0][0], R[0][1] + 1) or p[1] not in range(R[1][0], R[1][1] + 1) or p[2] not in range(R[2][0], R[2][1] + 1):
            break
          v.add(p)
          for D in DIRS:
